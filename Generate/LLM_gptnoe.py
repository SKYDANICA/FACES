...
def comment(example_source1, example_target1, example_source2,example_target2, example_source3, example_target3,example_source4, example_target4,example_source5,example_target5, example_source6, example_target6, example_source7,example_target7, example_source8, example_target8,example_source9, example_target9,example_source10,example_target10, code, num):#
    ...
    prompt_str = "You are a code expert. Please generate a brief summary of the code. To assist you, we will provide k examples for reference. Additionally, we have included test cases; please simulate the function execution to understand its internal logic：\n"
    ...
    if prompt_token.shape[1] > 2000:
        ...
        prompt_token = tokenizer.encode(prompt, return_tensors="pt")
    if prompt_token.shape[1] > 2000:
        prompt_token1 = tokenizer.encode(example_source6 , return_tensors="pt")[:,:768]
        s1 = tokenizer.encode("}<s>", return_tensors="pt")
        prompt_token = torch.cat((prompt_token1, s1), dim=1)
        prompt_token2 = tokenizer.encode(str(example_target6), return_tensors="pt")[:,:256]
        prompt_token = torch.cat((prompt_token, prompt_token2), dim=1)
        s2 = tokenizer.encode(".</s>\n\n", return_tensors="pt")
        prompt_token = torch.cat((prompt_token, s2), dim=1)
        prompt_token3 = tokenizer.encode(code, return_tensors="pt")[:,:768]
        prompt_token = torch.cat((prompt_token, prompt_token3), dim=1)   
        spe_token_ = tokenizer.encode("}<s>", return_tensors="pt").to(device)
...
if __name__ == '__main__':
    ...
    for batch_index in range(num_batches):
        ...
        #break
    with open(os.path.join("../", "gptnoe" + str(args.test_num) + ".output"), 'w', encoding='utf-8') as f:
        idx = 0
        for gold in all_:
            for line in gold:
